"""ORCH wiring — E02 passively consumes Feature Ready + E01 Ready + E14 Ready."""
...
def register_e02_with_orch(
    ...
    *,
    ...
    def _on_ready(ready: FeatureReadyEvent) -> None:
        ...
        try:
            e02.on_feature_ready(as_of=as_of, symbol=ready.symbol, snapshot=snapshot)
        except Exception as exc:
            log.warning("e02_feature_ready_failed", extra={"extra": {"error": str(exc)}})
...
def _wrap_e01(e01: E01Service, e02: E02Service) -> None:
    original = e01.run

    def _run(*args: Any, **kwargs: Any):
        state = original(*args, **kwargs)
        try:
            e02.on_e01_ready(state)
        except Exception as exc:
            log.warning("e02_after_e01_failed", extra={"extra": {"error": str(exc)}})
        return state

    e01.run = _run  # type: ignore[method-assign]


def _wrap_e14(e14: E14Service, e02: E02Service) -> None:
    original = e14.run

    def _run(*args: Any, **kwargs: Any):
        state = original(*args, **kwargs)
        try:
            e02.on_e14_ready(state)
        except Exception as exc:
            log.warning("e02_after_e14_failed", extra={"extra": {"error": str(exc)}})
        return state
