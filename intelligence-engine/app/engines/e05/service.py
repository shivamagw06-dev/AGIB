"""E05 service — FeatureSnapshot + E01/E14 + PIT events → E05EventState → EngineState."""
...
from app.engines.e05.event_state import E05EventState, e05_from_row
from app.engines.e05.features.builder import EventFeatureBuilder
from app.engines.e05.flags import E05Flags
from app.engines.e05.mapping import FORMULA_ID, MODEL_VERSION
from app.engines.e05.metrics import E05Metrics, Timer
from app.engines.e05.models.state import compute_universe_states
from app.engines.e05.state_builder import build_e05_engine_state
from app.engines.e05.store import E05StateStore
...
class E05Service:
    """Passive consumer. No MarketDataClient. No deal-prob / transcripts / ML / forecasting."""
    ...
    def __init__(
        ...
        *,
        ...
        store: E05StateStore | None = None,
        orch_ledger: OrchLedger | None = None,
        flags: E05Flags | None = None,
        default_universe_id: str = "NSE_INVESTABLE_L1",
    ) -> None:
        self.registry = registry
        self.e01 = e01
        self.e14 = e14
        self.builder = EventFeatureBuilder(registry)
        self.store = store or E05StateStore()
        self.orch_ledger = orch_ledger
        self.flags = flags or E05Flags.from_settings()
        self.metrics = E05Metrics()
    ...
    def run_universe(
        ...
        *,
        ...
        persist: bool = True,
    ) -> dict[str, E05EventState]:
        timer = Timer()
        if not self.flags.e05_p0:
            raise RuntimeError("E05_P0 is disabled")
        ...
        try:
                    ...
                    "events": [
                        {
                            ...
                            "guidance_delta": e.guidance_delta,
                        }
            ...
            for sym, row in rows.items():
                ...
                state = build_e05_engine_state(
                    evt,
                    generated_at=generated_at or datetime.now(timezone.utc),
                    flags=self._flag_map(),
                    confidence_value=min(1.0, evt.confidence * conf_adj),
                )
                errors = validate_engine_state(state.model_dump(mode="json"))
                if errors:
                    raise ValueError(f"E05EventState schema invalid for {sym}: {errors[:3]}")
                if persist:
                    self.store.put(evt, state)
                out[sym] = evt
    ...
    def get_event_state(self, symbol: str, as_of: str | None = None) -> E05EventState | None:
        timer = Timer()
        evt = self.store.get_event_state(symbol, as_of=as_of)
        self.metrics.record_lookup(timer.ms(), cache_hit=evt is not None)
        return evt
    ...
    def on_feature_ready(
        ...
        *,
        ...
        if symbol and snapshot is not None:
            ...
            return None
        log.info("e05_consume_feature_ready", extra={"extra": {"as_of": as_of, "symbol": symbol}})
        return self.run_universe(as_of=as_of, panels=panels, snapshots=snapshots)

    def on_e01_ready(self, e01_state: EngineState) -> dict[str, E05EventState] | None:
        if not self.flags.e05_p0 or not self._panels:
            return None
        return self.run_universe(as_of=e01_state.as_of, e01_state=e01_state)

    def on_e14_ready(self, e14_state: EngineState) -> dict[str, E05EventState] | None:
        if not self.flags.e05_p0 or not self._panels:
            return None
    ...
    def health(self) -> dict[str, Any]:
        return {
            "ok": self.flags.e05_p0,
            "service": "e05-event-driven-special-situations",
            "engine": "E05",
            ...
            "transcripts": False,
            "event_forecasting": False,
            "execution": False,
        }

    def _flag_map(self) -> dict[str, bool]:
        return {
            "E05_P0": self.flags.e05_p0,
            "E05_DEAL_PROBABILITY": self.flags.e05_deal_probability,
            "E05_TRANSCRIPTS": self.flags.e05_transcripts,
            "E05_ML": self.flags.e05_ml,
        }

    def _gate_placeholders(self) -> None:
        if self.flags.e05_deal_probability:
            from app.engines.e05.models import deal_probability as _d

            _d.deal_probability_disabled()
        if self.flags.e05_transcripts:
            from app.engines.e05.models import transcripts as _t

            _t.transcripts_disabled()
        if self.flags.e05_ml:
            from app.engines.e05.models import ml as _m
    ...
    def _record_orch(self, *, as_of: str, n: int, latency_ms: float, ok: bool) -> None:
        ...
        try:
            if "E05_EVENT" in self.orch_ledger.dag_node_ids():
