"""E08 service — FeatureSnapshot + E01/E14 → E08State → EngineState."""
...
from app.engines.e08.features.builder import VolatilityFeatureBuilder
from app.engines.e08.flags import E08Flags
from app.engines.e08.mapping import FORMULA_ID, MODEL_VERSION
from app.engines.e08.metrics import E08Metrics, Timer
from app.engines.e08.models.state import compute_universe_states
from app.engines.e08.state_builder import build_e08_engine_state
from app.engines.e08.store import E08StateStore
from app.engines.e08.vol_state import E08State, e08_from_row
...
class E08Service:
    """Passive consumer. No MarketDataClient. No dealer/gamma/surface/ML."""
    ...
    def __init__(
        ...
        *,
        ...
        store: E08StateStore | None = None,
        orch_ledger: OrchLedger | None = None,
        flags: E08Flags | None = None,
        default_universe_id: str = "NSE_INVESTABLE_L1",
    ) -> None:
        self.registry = registry
        self.e01 = e01
        self.e14 = e14
        self.builder = VolatilityFeatureBuilder(registry)
        self.store = store or E08StateStore()
        self.orch_ledger = orch_ledger
        self.flags = flags or E08Flags.from_settings()
        self.metrics = E08Metrics()
    ...
    def run_universe(
        ...
        *,
        ...
        timer = Timer()
        if not self.flags.e08_p0:
            raise RuntimeError("E08_P0 is disabled")
        ...
        try:
            ...
            for sym, row in rows.items():
                ...
                state = build_e08_engine_state(
                    vol,
                    generated_at=generated_at or datetime.now(timezone.utc),
                    flags=self._flag_map(),
                    confidence_value=min(1.0, vol.confidence * conf_adj),
                )
                errors = validate_engine_state(state.model_dump(mode="json"))
                if errors:
                    raise ValueError(f"E08State schema invalid for {sym}: {errors[:3]}")
                if persist:
                    self.store.put(vol, state)
                out[sym] = vol
    ...
    def get_vol_state(self, symbol: str, as_of: str | None = None) -> E08State | None:
        timer = Timer()
        vol = self.store.get_vol_state(symbol, as_of=as_of)
        self.metrics.record_lookup(timer.ms(), cache_hit=vol is not None)
        return vol
    ...
    def on_feature_ready(
        ...
        *,
        ...
        if symbol and snapshot is not None:
            ...
            return None
        log.info("e08_consume_feature_ready", extra={"extra": {"as_of": as_of, "symbol": symbol}})
        return self.run_universe(as_of=as_of, panels=panels, snapshots=snapshots)

    def on_e01_ready(self, e01_state: EngineState) -> dict[str, E08State] | None:
        if not self.flags.e08_p0 or not self._panels:
            return None
        return self.run_universe(as_of=e01_state.as_of, e01_state=e01_state)

    def on_e14_ready(self, e14_state: EngineState) -> dict[str, E08State] | None:
        if not self.flags.e08_p0 or not self._panels:
            return None
    ...
    def health(self) -> dict[str, Any]:
        return {
            "ok": self.flags.e08_p0,
            "service": "e08-volatility-options-intelligence",
            "engine": "E08",
            "node_id": self.NODE_ID,
            "flags": self._flag_map(),
            "store": self.store.stats(),
            "metrics": self.metrics.snapshot(),
            "consumes": ["FeatureSnapshot", "E01State", "E14State", "VOL_*", "OPTIONS_*"],
            "market_data_access": False,
            "polling": False,
            "formula_id": FORMULA_ID,
            "ml": False,
            "dealer_positioning": False,
            "options_surface": False,
            "gamma_exposure_model": False,
        }

    def _flag_map(self) -> dict[str, bool]:
        return {
            "E08_P0": self.flags.e08_p0,
            "E08_GAMMA": self.flags.e08_gamma,
            "E08_DEALER": self.flags.e08_dealer,
            "E08_SURFACE": self.flags.e08_surface,
            "E08_ML": self.flags.e08_ml,
        }

    def _gate_placeholders(self) -> None:
        if self.flags.e08_gamma:
            from app.engines.e08.models import gamma as _g

            _g.gamma_disabled()
        if self.flags.e08_dealer:
            from app.engines.e08.models import dealer as _d

            _d.dealer_disabled()
        if self.flags.e08_surface:
            from app.engines.e08.models import surface as _s

            _s.surface_disabled()
        if self.flags.e08_ml:
            from app.engines.e08.models import ml as _m
    ...
    def _record_orch(self, *, as_of: str, n: int, latency_ms: float, ok: bool) -> None:
        ...
        try:
            if "E08_VOL" in self.orch_ledger.dag_node_ids():
