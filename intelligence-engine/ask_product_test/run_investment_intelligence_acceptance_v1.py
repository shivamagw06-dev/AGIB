#!/usr/bin/env python3
"""Run Investment Intelligence Acceptance Test v1.0 (engine in-process).
...
from ask_product_test.investment_intelligence_acceptance_v1 import (  # noqa: E402
    INV_ACCEPTANCE_300,
    evaluate_inv_case,
)
from investment_intelligence.production import analyse, health  # noqa: E402


def main() -> int:
    h = health()
    print(
        f"[inv_acceptance_v1] version={h.get('version')} "
    ...
    rows = []
    for i, case in enumerate(INV_ACCEPTANCE_300, 1):
        ...
        try:
            ...
            payload = {
                ...
                "modules_used": [],
                "fabricated": False,
                "recommendation": "BUY",  # force fail if exception path
                "error": str(exc),
            }
        row = evaluate_inv_case(case, payload)
        rows.append(row)
        print(
    ...
    report = {
        "suite": "Investment Intelligence Acceptance Test v1.0",
    ...
    (out / "investment_intelligence_acceptance_v1.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    print(
        f"\n[inv_acceptance_v1] {passed}/{len(rows)} ({rate}%) "
