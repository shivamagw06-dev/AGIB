#!/usr/bin/env python3
"""Run Timeline Regression for Research Intelligence. Gate: 100%."""
...
from ask_product_test.research_timeline_regression import (  # noqa: E402
    TIMELINE_REGRESSION,
    evaluate_timeline_case,
)
...
def main() -> int:
    ...
    for i, case in enumerate(TIMELINE_REGRESSION, 1):
        print(f"[{i}/{len(TIMELINE_REGRESSION)}] {case['id']} — {case['prompt'][:72]}", flush=True)
        payload = analyse(case["prompt"], entity=case["entity"])
        row = evaluate_timeline_case(case, payload)
        rows.append(row)
    ...
    report = {
        "suite": "Research Timeline Regression",
    ...
    for out in (Path("/workspace/artifacts"), Path("/opt/cursor/artifacts")):
        try:
            ...
            (out / "research_timeline_regression.json").write_text(
                json.dumps(report, indent=2) + "\n", encoding="utf-8"
            )
        except OSError:
            pass
    print(f"\n[timeline_regression] {passed}/{len(rows)} ({rate}%) {decision}", flush=True)
    for r in rows:
        if not r["pass"]:
