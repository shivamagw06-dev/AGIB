"""LangSmith-oriented CTI traces — deterministic span records (no LLM required)."""
...
def end(span: dict[str, Any], *, ok: bool = True, output: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    span["output_summary"] = {
        k: output[k]
        for k in ("count", "triggered", "watching", "scenario_impacts")
        if output and k in output
    }
    # Drop internal mono clock from persisted view
