...
def test_benchmark_registry_has_500_questions():
...
def test_benchmark_categories():
    valuation = list_benchmarks(category="valuation", limit=50)
    assert valuation
    assert all(q["category"] == "valuation" for q in valuation)
    assert get_benchmark("IWB_001")["question"] == "Should I invest in TCS?"
...
def test_apply_editorial_excellence_wiring():
    base = apply_institutional_writing_constitution(
        {"ticker": "TCS", "company": "TCS", "query": "Should I invest in TCS?"},
        query="Should I invest in TCS?",
    )
    out = apply_editorial_excellence(base, query="Should I invest in TCS?", benchmark_id="IWB_001")
...
def test_hall_of_fame_keeps_better_version():
    ...
    assert hof["entries"]["IWB_TEST"]["editorial_score"] == 92.0
