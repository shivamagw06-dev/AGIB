...
def governance_diff(ctx: dict[str, Any]) -> list[ChangeRecord]:
    cur_p = ctx.get("current_period")
    prev_p = ctx.get("previous_period")
    if not cur_p or not prev_p:
        return []
    qual = ctx.get("qual_by_period") or {}
    cur = set((qual.get(cur_p) or {}).get("governance") or [])
    prev = set((qual.get(prev_p) or {}).get("governance") or [])
    ticker = ctx["ticker"]
    out: list[ChangeRecord] = []
    for item in sorted(cur ^ prev):
        added = item in cur
        change_type = "governance_added" if added else "governance_removed"
        if "CEO" in item:
            change_type = "ceo_change"
        cause = explain_change(
            metric=item,
            change_type=change_type,
            previous="present" if not added else "absent",
        ...
        out.append(
            ChangeRecord(
                change_id=f"{ticker}:gov:{change_type}:{item}",
                ticker=ticker,
                domain="governance",
                metric=item,
                change_type=change_type,
                ...
                section="governance",
                materiality="critical" if "CEO" in item or "Auditor" in item else "medium",
