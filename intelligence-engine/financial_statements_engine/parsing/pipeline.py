"""PNE pipeline — Raw Evidence → Canonical Statement drafts (not warehouse)."""
...
def parse_document(
    *,
    ...
    """Run full PNE pipeline. Never publishes to Financial Warehouse."""
    assert WRITES_WAREHOUSE is False
    meta = dict(meta or {})
    t = ticker.upper().strip()
    publish("parse.started", {"ticker": t, "evidence_id": evidence_id})
    ...
    if parser is None:
        ...
        publish("parse.quarantined", {"ticker": t, "evidence_id": evidence_id, "reason": "unsupported_format"})
        return {"ok": False, "quarantined": True, "quarantine": q, "writes_warehouse": False}

    extracted = parser.parse(data, meta={**meta, **identity})
    if extracted.get("quarantine") or (not extracted.get("ok") and "unsupported_format" in (extracted.get("errors") or [])):
    ...
    # Build CFDM drafts — only when period is known; otherwise draft without period publish identity
    ...
    if period_info.get("period_end") and period_info.get("period_kind"):
        ...
        # Group metrics by statement_type from registry record
        ...
        for stmt_type, metrics in sorted(by_stmt.items()):
            ...
            for metric, row in sorted(metrics.items()):
                ...
                # Attach parse trace extras (not altering values)
                fact["trace"] = {
    ...
    # Deterministic core excludes wall-clock timestamps
    deterministic_core = {
        "ticker": t,
        "evidence_id": evidence_id,
        "document_type": identity["document_type"],
    ...
    # Persist draft (not warehouse published/)
    draft_dir = ensure_dirs() / "parsing" / "drafts" / t
    draft_dir.mkdir(parents=True, exist_ok=True)
    draft_path = draft_dir / f"{evidence_id.replace(':', '_')}.json"
