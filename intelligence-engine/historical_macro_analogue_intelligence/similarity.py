"""Deterministic multi-dimension similarity scoring for macro analogues."""
...
# Weights must sum to 1.0 — every dimension contributes to overall score.
...
DIMENSION_SCALES: dict[str, float] = {
    ...
    "gdp": 4.0,  # pp GDP
    "liquidity": 2.5,  # banking liquidity units
    "fiscal": 3.0,  # pp fiscal deficit
    "currency": 8.0,  # USDINR points (or FX reserves bn / 50 scaled)
    "bond_yield": 2.0,  # pp G-Sec
    "global_growth": 2.5,  # pp WEO
    "commodity": 6.0,  # WPI / crude pressure proxy
}

DIMENSION_LABELS: dict[str, str] = {
    "interest_rate": "Interest-Rate Regime",
    "inflation": "Inflation Regime",
    "gdp": "GDP Regime",
    "liquidity": "Liquidity Regime",
    "fiscal": "Fiscal Regime",
    "currency": "Currency Regime",
    "bond_yield": "Bond Yield Regime",
    "global_growth": "Global Growth Regime",
    "commodity": "Commodity Regime",
}
...
def score_dimensions(
    ...
    *,
    ...
    for dim in SIMILARITY_DIMENSIONS:
        ...
        # If either side missing, skip weight renormalization contribution
...
def confidence_for(score: float, *, evidence_n: int, dimensions_scored: int) -> ConfidenceLabel:
    if score >= 85 and evidence_n >= 2 and dimensions_scored >= 5:
        return "High"
...
def key_differences(details: list[DimensionScore], *, max_n: int = 4) -> list[str]:
    """Explain largest absolute gaps between current and historical values."""
