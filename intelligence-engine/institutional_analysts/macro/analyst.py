...
def analyse(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    transmission = (
        scrub_public(briefing.get("macro_transmission") or macro.get("transmission") or macro_layer.get("reasoning"), limit=260)
    ...
    return opinion(
        role="macro",
        question="Does macro help or hurt?",
        headline=f"Macro backdrop for {name}: rates, inflation, growth, and liquidity set the external constraint.",
        sections={
            ...
            "macro_outlook": briefing.get("current_outlook") or macro.get("outlook") or macro_layer.get("reasoning") or "Outlook remains data-dependent",
            "transmission": transmission,
            "drivers": briefing.get("macro_drivers") or macro.get("drivers") or ["Rates", "Growth", "Liquidity"],
        },
        evidence=evidence,
        confidence=pick_confidence(macro.get("confidence"), macro_layer.get("score"), default=0.55),
        score=macro_layer.get("score"),
        word_limit=450,
    )
