...
here:
    ...
    normalise -> validate -> missing-value intelligence -> quality -> conflicts
             -> persist -> audit
...
from institutional_warehouse import audit, conflicts, db, missing_values, quality, store, validation
...
def write(
    ...
    *,
    ...
    # 1. Missing-value intelligence, before validation sees the row: a zero that
    ...
    # 2. Validation. Rejected rows are quarantined, never silently dropped.
    ...
    # 3. Conflict detection against what is already stored.
    ...
    # 5. Quality metadata for the rows that landed.
