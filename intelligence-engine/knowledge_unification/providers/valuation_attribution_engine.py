...
class ValuationAttributionEngineProvider:
    ...
    def consult(self, plan: QueryPlan) -> ProviderResult:
        ...
        if not pack.get("ok"):
            return empty_result(self.spec.id, t0, str(pack.get("error") or "varie_empty"))

        snap = pack.get("snapshot") or {}
        summary = pack.get("research_note") or pack.get("summary") or (
            f"{ticker} valuation attribution — premium {snap.get('premium_pct')}% "
            f"vs history (percentile {snap.get('historical_percentile')})."
        )
        why = list(pack.get("why") or [])[:8]
        expl = pack.get("explainability") or {
            "observed": why[:3],
            "derived": [f"premium_pct={snap.get('premium_pct')}"],
            "inferred": [str((pack.get("research_note") or ""))[:240]],
        }
        return timed_result(
            self.spec.id,
            ok=True,
            empty=False,
            confidence=float((pack.get("confidence") or {}).get("score") or 0.7)
            if isinstance(pack.get("confidence"), dict)
            else 0.7,
            t0=t0,
            ...
            raw={
                ...
                "symbol": ticker,
                "factors": (pack.get("factors") or [])[:8],
                "peer": peer_pack if peer_pack and peer_pack.get("ok") else None,
            },
        )
