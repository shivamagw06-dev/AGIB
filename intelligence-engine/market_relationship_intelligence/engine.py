"""MKRI engine — discover, validate, score, publish, retrieve market relationships."""
...
from market_relationship_intelligence.discovery import (
    discover_from_catalog,
    enrich_with_hmip,
    enrich_with_hmkip,
    enrich_with_hsip,
    enrich_with_macro_mri_tip,
)
from market_relationship_intelligence.graph import build_graph, find_paths
...
from market_relationship_intelligence.validation import (
    is_stale,
    score_confidence,
...
class MarketRelationshipIntelligenceEngine:
    def health(self) -> dict[str, Any]:
        return {
            "status": "ok",
            "programme": PROGRAMME,
            "programme_short": PROGRAMME_SHORT,
            "version": MKRI_VERSION,
            "principle": PRIMARY_PRINCIPLE,
            "does_not": list(NO_MKRI_ACTIONS),
            "ask_triggers_collection": False,
            "providers_queried_always": [],
            "consumes": ["HMKIP", "HMIP", "HSIP", "Macro MRI tips", "CMKTP", "Research"],
            "phase": "12.3",
            "preceded_by": ["CMKTP 12.1", "HMKIP 12.2"],
            "enables": ["HMAI 12.4", "MFI 12.5"],
            "note": "Programme short MKRI avoids collision with Macro MRI",
        }
    ...
    def run(
        ...
        *,
        ...
        """Ops rebuild — discover → enrich → validate → score → publish. Never Ask."""
        span = traces.begin(
        ...
        for rel in candidates:
            if enrich_hmkip:
                rel = enrich_with_hmkip(rel)
            if enrich_hmip:
                rel = enrich_with_hmip(rel)
            if enrich_hsip:
                rel = enrich_with_hsip(rel)
            if enrich_macro_mri:
                rel = enrich_with_macro_mri_tip(rel)
            ...
            if errors:
                ...
                rejected += 1
                continue
            traces.end(vspan, output={"ok": True, "evidence_n": len(rel.evidence)})
            ...
            traces.end(
                ...
                output={
                    ...
                    "stale": rel.stale,
                    "historical_observations": rel.historical_observations,
                },
            )
        ...
        gspan = traces.begin("market_relationship_graph", meta={"published": published})
        graph = build_graph()
        traces.end(gspan, output={"nodes": graph["n_nodes"], "edges": graph["n_edges"]})

        rspan = traces.begin("market_relationship_refresh", meta={"published": published})
        summary = {
            "ok": True,
            "published": published,
            "rejected": rejected,
    ...
    def relationships(self, *, limit: int = 200) -> dict[str, Any]:
        span = traces.begin("market_relationship_retrieval", meta={"scope": "all"})
    ...
    def for_indicator(self, indicator: str, *, limit: int = 100) -> dict[str, Any]:
        ...
        rows = STORE.for_endpoint(indicator, limit=limit)
        out = {
            "indicator": indicator,
            "n": len(rows),
            "relationships": [r.to_public_dict() for r in rows],
    ...
    def graph(self, *, start: str | None = None, end: str | None = None) -> dict[str, Any]:
        span = traces.begin("market_relationship_graph", meta={"source": "api", "start": start})
        g = build_graph()
        if start:
    ...
    def dashboard(self) -> dict[str, Any]:
        ...
        g = build_graph() if rows else {"n_nodes": 0, "n_edges": 0}

        return {
            "board": "Market Relationship Intelligence",
            "programme": PROGRAMME,
            "programme_short": PROGRAMME_SHORT,
            "version": MKRI_VERSION,
            "principles": {
            ...
            "relationship_coverage": {
                "markets": cov.get("markets_covered"),
                "sectors": cov.get("sectors_covered"),
                "asset_classes": cov.get("asset_classes_covered"),
            ...
            "graph_health": {
                "nodes": g.get("n_nodes"),
                "edges": g.get("n_edges"),
                "transmission_paths": len(g.get("transmission_paths") or []) if isinstance(g, dict) else 0,
            },
            "coverage_by_market_sector_asset": {
                "markets": cov.get("markets_covered"),
                "sectors": cov.get("sectors_covered"),
                "asset_classes": cov.get("asset_classes_covered"),
            },
            ...
            "phase": "12.3",
            "note": "Read APIs never rebuild the graph. Use POST /v1/market/relationships/run.",
        }
