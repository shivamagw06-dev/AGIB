"""Sector Forecast Intelligence engine — Bull/Base/Bear from AGI sector knowledge."""

from __future__ import annotations

from typing import Any

from sector_forecast_intelligence import traces
from sector_forecast_intelligence.bundle import assemble_bundle, normalize_sector
from sector_forecast_intelligence.impacts import (
    company_impacts_for,
    impact_matrices,
    peer_impacts_for,
)
from sector_forecast_intelligence.probability import assign_probabilities, score_confidence
from sector_forecast_intelligence.schema import (
    FORECAST_DIMENSIONS,
    NO_SFI_ACTIONS,
    PRIMARY_PRINCIPLE,
    PROGRAMME,
    PROGRAMME_SHORT,
    SFI_VERSION,
    SUPPORTED_SECTORS,
    ScenarioType,
    SectorForecastReport,
    SectorMetricForecast,
    SectorScenario,
)
from sector_forecast_intelligence.store import STORE
from sector_forecast_intelligence.templates import (
    UNITS,
    assumptions_for,
    base_levels_for,
    drivers_for,
    narratives_for,
    path_deltas,
    risks_for,
)


class SectorForecastIntelligenceEngine:
    def health(self) -> dict[str, Any]:
        return {
            "status": "ok",
            "programme": PROGRAMME,
            "programme_short": PROGRAMME_SHORT,
            "version": SFI_VERSION,
            "principle": PRIMARY_PRINCIPLE,
            "does_not": list(NO_SFI_ACTIONS),
            "ask_triggers_collection": False,
            "providers_queried_always": [],
            "consumes": ["CSKP", "HSIP", "SRI", "HSAI", "MFI (macro inheritance)"],
            "feeds": ["Investment Office", "Company Intelligence", "Portfolio Intelligence"],
            "phase": "11.5",
            "preceded_by": ["CSKP 11.1", "HSIP 11.2", "SRI 11.3", "HSAI 11.4"],
            "predicts_single_path": False,
            "inherits_macro_from": "MFI",
            "supported_sectors": list(SUPPORTED_SECTORS),
            "forecast_dimensions": list(FORECAST_DIMENSIONS),
        }

    def run(self, *, sector: str | None = None, country: str = "India") -> dict[str, Any]:
        """Ops publish — assemble + generate + publish. Never Ask."""
        sectors = [normalize_sector(sector) or sector] if sector else list(SUPPORTED_SECTORS)
        published = 0
        per: dict[str, Any] = {}
        for sec in sectors:
            if not sec:
                continue
            report = self.generate_report(sector=sec, country=country, persist=True)
            published += 1
            per[sec] = {
                "report_id": report.report_id,
                "version": report.version,
                "probability_distribution": report.probability_distribution,
                "confidence_pct": (report.confidence or {}).get("overall_pct"),
            }
        summary = {
            "ok": True,
            "sectors": list(per.keys()),
            "published": published,
            "per_sector": per,
            "ask_triggered": False,
            "providers_queried": [],
            "scenarios": ["Bull", "Base", "Bear"],
        }
        STORE.record_run(summary)
        return summary

    def generate_report(
        self,
        *,
        sector: str = "Banking",
        country: str = "India",
        persist: bool = False,
    ) -> SectorForecastReport:
        sec = normalize_sector(sector) or sector
        bundle = assemble_bundle(sector=sec, country=country)
        scenarios = self._generate_scenarios(bundle)
        dist = self._assign_probability(scenarios, bundle)
        confidence = self._score_confidence(scenarios, bundle)
        company_matrix, peer_matrix = impact_matrices(scenarios)

        vspan = traces.begin(
            "sector_forecast_validation",
            meta={"sector": sec, "completeness": bundle.completeness_pct},
        )
        contradictions = self._contradictions(scenarios, bundle)
        traces.end(vspan, output={"contradictions": len(contradictions)})

        major_risks: list[dict[str, Any]] = []
        seen: set[str] = set()
        for sc in scenarios:
            for r in sc.risks:
                key = str(r.get("risk"))
                if key in seen:
                    continue
                seen.add(key)
                major_risks.append({**r, "scenario_context": sc.scenario})

        report = SectorForecastReport(
            sector=sec,
            country=country,
            horizon=bundle.horizon,
            bundle_id=bundle.bundle_id,
            current_outlook={
                "sector": sec,
                "outlook": (bundle.current_sector or {}).get("outlook"),
                "latest": (bundle.current_sector or {}).get("latest"),
                "completeness_pct": bundle.completeness_pct,
            },
            current_regime=bundle.current_regime,
            scenarios=scenarios,
            probability_distribution=dist,
            confidence=confidence,
            company_impact_matrix=company_matrix,
            peer_sector_impact_matrix=peer_matrix,
            key_catalysts=self._catalysts(bundle, scenarios),
            major_risks=major_risks[:12],
            macro_inheritance={
                "gateway": "MFI_KRIG",
                "inherited": bool((bundle.macro_forecast_tip or {}).get("inherited")),
                "probability_distribution": (bundle.macro_forecast_tip or {}).get(
                    "probability_distribution"
                ),
                "scenarios": (bundle.macro_forecast_tip or {}).get("scenarios") or [],
            },
            contradictions=contradictions,
            providers_queried=[],
            provenance={
                "sources": bundle.sources,
                "completeness_pct": bundle.completeness_pct,
                "analogue_n": len(bundle.analogues),
                "relationship_n": len(bundle.relationships),
            },
        )

        if persist:
            pspan = traces.begin(
                "sector_forecast_publication",
                meta={"sector": sec, "bundle_id": bundle.bundle_id},
            )
            published = STORE.publish(report)
            traces.end(
                pspan,
                output={
                    "report_id": published.report_id,
                    "version": published.version,
                    "distribution": published.probability_distribution,
                },
            )
            return published
        return report

    def _generate_scenarios(self, bundle) -> list[SectorScenario]:
        span = traces.begin(
            "sector_scenario_generation",
            meta={"sector": bundle.sector, "horizon": bundle.horizon},
        )
        defaults = base_levels_for(bundle.sector)
        # Soft overlay from HSAI regime features when present
        features = (bundle.current_regime or {}).get("features") or {}
        overlay: dict[str, float] = {}
        mapping = {
            "revenue_growth": "Revenue Growth",
            "earnings_growth": "Earnings Growth",
            "margin_profile": "EBITDA Margin",
            "roe": "ROE",
            "valuation": "PE",
            "relative_performance": "Relative Performance",
        }
        for k, metric in mapping.items():
            if features.get(k) is not None:
                try:
                    overlay[metric] = float(features[k])
                except (TypeError, ValueError):
                    pass
        base_levels = {**defaults, **overlay}

        # Macro inheritance notes
        macro_scenarios = {
            s.get("scenario"): s
            for s in ((bundle.macro_forecast_tip or {}).get("scenarios") or [])
        }

        scenarios: list[SectorScenario] = []
        for stype in ("Bull", "Base", "Bear"):
            scenario_type: ScenarioType = stype  # type: ignore[assignment]
            deltas = path_deltas(scenario_type)
            metrics: list[SectorMetricForecast] = []
            for name, delta in deltas.items():
                level = float(base_levels.get(name, defaults.get(name, 0.0))) + float(delta)
                level = round(level, 2)
                metrics.append(
                    SectorMetricForecast(
                        metric=name,
                        unit=UNITS.get(name, ""),
                        value=level,
                        note=f"{stype} path delta {delta:+}",
                    )
                )

            evidence = [
                {
                    "kind": "continuous_sector",
                    "summary": "Current sector tip from CSKP",
                    "refs": ["CSKP_KRIG"],
                },
                {
                    "kind": "research",
                    "summary": (bundle.research or {})
                    .get("sector_research_office", {})
                    .get("stance")
                    or "Sector research tip",
                    "refs": ["Sector_Research_Tip"],
                },
                {
                    "kind": "macro_inheritance",
                    "summary": "Macro assumptions inherited from MFI",
                    "refs": ["MFI_KRIG"],
                },
            ]
            analogues = []
            for a in bundle.analogues[:4]:
                analogues.append(
                    {
                        "matched_period": a.get("matched_period"),
                        "similarity_score": a.get("similarity_score"),
                        "label": a.get("matched_label"),
                        "outcome": a.get("historical_outcome"),
                        "equity_outcome": a.get("equity_outcome"),
                        "historical_outcome_bundle": a.get("historical_outcome_bundle"),
                    }
                )
                evidence.append(
                    {
                        "kind": "historical_analogue",
                        "summary": f"Analogue {a.get('matched_period')}: {a.get('matched_label')}",
                        "refs": a.get("timeline_refs") or ["HSAI_KRIG"],
                    }
                )
            if bundle.historical_tip.get("available"):
                evidence.append(
                    {
                        "kind": "historical_sector",
                        "summary": "HSIP series / timeline coverage",
                        "refs": ["HSIP_KRIG"],
                    }
                )

            rel_sample = [
                {
                    "source": r.get("source"),
                    "target": r.get("target"),
                    "relationship": r.get("relationship"),
                    "confidence_pct": r.get("confidence_pct"),
                    "kind": r.get("kind"),
                }
                for r in bundle.relationships[:8]
            ]

            cspan = traces.begin(
                "sector_company_impact", meta={"scenario": stype, "sector": bundle.sector}
            )
            companies = company_impacts_for(
                bundle.sector, scenario_type, relationships=bundle.relationships
            )
            peers = peer_impacts_for(
                bundle.sector, scenario_type, relationships=bundle.relationships
            )
            traces.end(cspan, output={"companies": len(companies), "peers": len(peers)})

            msc = macro_scenarios.get(stype) or {}
            macro_drivers = []
            if msc:
                macro_drivers = [
                    {"driver": "Repo Rate path", "value": msc.get("repo_rate"), "source": "MFI"},
                    {"driver": "CPI path", "value": msc.get("inflation"), "source": "MFI"},
                    {"driver": "GDP path", "value": msc.get("gdp"), "source": "MFI"},
                    {"driver": "USDINR path", "value": msc.get("usdinr"), "source": "MFI"},
                ]

            catalysts = [
                {
                    "catalyst": f"{bundle.sector} demand inflection",
                    "polarity": "positive" if stype == "Bull" else ("negative" if stype == "Bear" else "mixed"),
                },
                {
                    "catalyst": "Macro policy transmission (inherited)",
                    "polarity": "positive" if stype != "Bear" else "negative",
                },
                {
                    "catalyst": "Margin / valuation digest",
                    "polarity": "watch" if stype == "Base" else ("positive" if stype == "Bull" else "negative"),
                },
            ]

            scenarios.append(
                SectorScenario(
                    scenario=scenario_type,
                    sector=bundle.sector,
                    country=bundle.country,
                    forecast_horizon=bundle.horizon,
                    metrics=metrics,
                    narrative=narratives_for(bundle.sector, scenario_type),
                    drivers=drivers_for(bundle.sector, scenario_type),
                    catalysts=catalysts,
                    risks=risks_for(scenario_type),
                    key_assumptions=assumptions_for(scenario_type),
                    supporting_evidence=evidence,
                    historical_analogues=analogues,
                    supporting_relationships=rel_sample,
                    macro_drivers=macro_drivers,
                    company_impacts=companies,
                    peer_sector_impacts=peers,
                    provenance={
                        "base_levels_source": "HSAI_regime" if overlay else "catalog_defaults",
                        "macro_inherited": bool(msc),
                    },
                )
            )

        traces.end(
            span,
            output={
                "scenarios": [s.scenario for s in scenarios],
                "metrics_per": len(scenarios[0].metrics) if scenarios else 0,
            },
        )
        return scenarios

    def _assign_probability(self, scenarios, bundle) -> dict[str, int]:
        span = traces.begin("sector_probability", meta={"sector": bundle.sector})
        dist = assign_probabilities(scenarios, bundle)
        traces.end(span, output={"distribution": dist, "sum": sum(dist.values())})
        return dist

    def _score_confidence(self, scenarios, bundle) -> dict[str, Any]:
        span = traces.begin(
            "sector_confidence", meta={"completeness": bundle.completeness_pct}
        )
        conf = score_confidence(scenarios, bundle)
        traces.end(span, output={"overall_pct": conf.get("overall_pct"), "label": conf.get("label")})
        return conf

    def _catalysts(self, bundle, scenarios) -> list[dict[str, Any]]:
        cats: list[dict[str, Any]] = []
        seen: set[str] = set()
        for sc in scenarios:
            for c in sc.catalysts:
                key = str(c.get("catalyst"))
                if key in seen:
                    continue
                seen.add(key)
                cats.append({**c, "scenario_context": sc.scenario})
        for m in bundle.monitoring[:5]:
            cats.append(
                {
                    "catalyst": m.get("event"),
                    "polarity": "watch",
                    "status": m.get("status"),
                    "importance": m.get("importance"),
                }
            )
        return cats[:12]

    def _contradictions(self, scenarios, bundle) -> list[str]:
        out: list[str] = []
        bull = next(s for s in scenarios if s.scenario == "Bull")
        bear = next(s for s in scenarios if s.scenario == "Bear")
        b_rev = bull.metric_map().get("Revenue Growth")
        r_rev = bear.metric_map().get("Revenue Growth")
        if b_rev is not None and r_rev is not None and b_rev > r_rev:
            out.append("Bull assumes higher revenue growth than Bear — demand path is the key fork")
        b_pe = bull.metric_map().get("PE")
        r_pe = bear.metric_map().get("PE")
        if b_pe is not None and r_pe is not None and b_pe > r_pe + 2:
            out.append("Bull valuation path materially above Bear — multiple expansion risk")
        if bundle.analogues:
            top = bundle.analogues[0]
            if float(top.get("similarity_score") or 0) < 70:
                out.append("Top historical analogue similarity below 70 — regime match is imperfect")
        if bundle.completeness_pct < 70:
            out.append("Sector knowledge completeness below 70% — widen confidence bands")
        if not (bundle.macro_forecast_tip or {}).get("inherited"):
            out.append("Macro Forecast tip unavailable — sector scenarios use catalog macro neutrality")
        return out

    # --- Retrieval surfaces ---

    def forecast(self, *, sector: str = "Banking") -> dict[str, Any]:
        sec = normalize_sector(sector) or sector
        latest = STORE.latest(sector=sec)
        if latest:
            return {
                **latest.to_public_dict(),
                "mode": "published",
                "gateway": "SFI_KRIG",
            }
        report = self.generate_report(sector=sec, persist=False)
        return {**report.to_public_dict(), "mode": "computed", "gateway": "SFI_KRIG"}

    def forecast_all(self, *, limit: int = 20) -> dict[str, Any]:
        rows = []
        for sec in SUPPORTED_SECTORS[:limit]:
            pack = self.forecast(sector=sec)
            rows.append(
                {
                    "sector": sec,
                    "probability_distribution": pack.get("probability_distribution"),
                    "confidence_pct": (pack.get("confidence") or {}).get("overall_pct"),
                    "mode": pack.get("mode"),
                    "version": pack.get("version"),
                }
            )
        return {
            "n": len(rows),
            "forecasts": rows,
            "providers_queried": [],
            "collected_on_request": False,
            "gateway": "SFI_KRIG",
            "predicts_single_path": False,
        }

    def scenarios(self, *, sector: str = "Banking") -> dict[str, Any]:
        pack = self.forecast(sector=sector)
        return {
            "sector": normalize_sector(sector) or sector,
            "n": len(pack.get("scenarios") or []),
            "scenarios": pack.get("scenarios") or [],
            "probability_distribution": pack.get("probability_distribution"),
            "providers_queried": [],
            "collected_on_request": False,
            "gateway": "SFI_KRIG",
        }

    def probability(self, *, sector: str = "Banking") -> dict[str, Any]:
        pack = self.forecast(sector=sector)
        return {
            "sector": normalize_sector(sector) or sector,
            "distribution": pack.get("probability_distribution") or {},
            "sum_pct": sum((pack.get("probability_distribution") or {}).values()),
            "confidence": pack.get("confidence"),
            "scenario_probabilities": [
                {
                    "scenario": s.get("scenario"),
                    "probability_pct": s.get("probability_pct"),
                    "confidence_pct": s.get("confidence_pct"),
                }
                for s in pack.get("scenarios") or []
            ],
            "providers_queried": [],
            "gateway": "SFI_KRIG",
            "note": "Probability quantifies scenario likelihood; confidence quantifies assessment certainty.",
        }

    def report(self, *, sector: str = "Banking", persist: bool = False) -> dict[str, Any]:
        if persist:
            r = self.generate_report(sector=sector, persist=True)
            return {**r.to_public_dict(), "mode": "published", "gateway": "SFI_KRIG"}
        return self.forecast(sector=sector)

    def history(self, *, sector: str | None = None, limit: int = 20) -> dict[str, Any]:
        sec = normalize_sector(sector) if sector else None
        rows = STORE.history(limit=limit, sector=sec)
        return {
            "sector": sec,
            "n": len(rows),
            "reports": [
                {
                    "report_id": r.report_id,
                    "sector": r.sector,
                    "version": r.version,
                    "published_at": r.published_at.isoformat() if r.published_at else None,
                    "probability_distribution": r.probability_distribution,
                    "confidence_pct": (r.confidence or {}).get("overall_pct"),
                }
                for r in rows
            ],
            "providers_queried": [],
            "gateway": "SFI_KRIG",
        }

    def dashboard(self) -> dict[str, Any]:
        latest = STORE.latest()
        if not latest:
            latest = self.generate_report(sector="Banking", persist=False)
            published = False
        else:
            published = True
        pub = latest.to_public_dict()
        return {
            "board": "Sector Forecast Intelligence",
            "programme": PROGRAMME,
            "version": SFI_VERSION,
            "principles": {
                "agi_owned_knowledge_only": True,
                "no_external_providers": True,
                "no_single_path_prediction": True,
                "ask_never_fetches": True,
                "versioned_reports": True,
                "evidence_linked_scenarios": True,
                "inherits_macro_from_mfi": True,
            },
            "does_not": list(NO_SFI_ACTIONS),
            "current_sector_outlook": pub.get("current_outlook"),
            "bull_base_bear_scenarios": pub.get("scenarios"),
            "probability_distribution": pub.get("probability_distribution"),
            "confidence": pub.get("confidence"),
            "confidence_trend": [
                {
                    "version": r.version,
                    "sector": r.sector,
                    "overall_pct": (r.confidence or {}).get("overall_pct"),
                }
                for r in STORE.history(limit=10)
            ],
            "key_catalysts": pub.get("key_catalysts"),
            "major_risks": pub.get("major_risks"),
            "company_impact_summaries": pub.get("company_impact_matrix"),
            "peer_sector_impacts": pub.get("peer_sector_impact_matrix"),
            "macro_inheritance": pub.get("macro_inheritance"),
            "forecast_revisions": self.history(limit=10),
            "forecast_accuracy_over_time": {
                "note": "Accuracy tracking reserved for post-publication outcome loops",
                "versions_tracked": STORE.coverage().get("latest_version"),
            },
            "coverage": STORE.coverage(),
            "retrieval_performance": {"traces": traces.recent(40)},
            "recent_runs": STORE.recent_runs(10),
            "ingestion_idle": not published and STORE.coverage()["total_reports"] == 0,
            "phase": "11.5",
            "providers_queried": [],
            "published": published,
        }
