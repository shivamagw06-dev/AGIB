"""SRI contracts — Sector Relationship Intelligence (Sprint 11.3)."""
...
SRI_VERSION = "0.1.0"
PROGRAMME = "Sector Relationship Intelligence"
PROGRAMME_SHORT = "SRI"
PRIMARY_PRINCIPLE = (
    "Relationships must always be evidence-backed, versioned and explainable. "
    "SRI never relies on hard-coded rules without supporting historical evidence."
)
...
def new_id(prefix: str = "sri") -> str:
    return f"{prefix}_{uuid4().hex[:12]}"


def stable_relationship_id(source: str, target: str, relationship: str) -> str:
    return str(uuid5(NAMESPACE_URL, f"sri:{source}:{target}:{relationship}"))


class RelationshipEvidence(BaseModel):
    kind: str  # historical_sector | historical_macro | historical_company | historical_market | research | timeline
    summary: str
...
class SectorRelationship(BaseModel):
    relationship_id: str = Field(default_factory=lambda: new_id("srel"))
    source: str
    ...
    programme_version: str = SRI_VERSION
...
class GraphNode(BaseModel):
    node_id: str
    label: str
    node_type: str  # macro | sector | industry | company | market | theme | global | policy
