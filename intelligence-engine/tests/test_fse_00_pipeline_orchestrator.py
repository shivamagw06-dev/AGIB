...
from financial_statements_engine.orchestrator.production import dashboard, health, history, queue, workflows
from financial_statements_engine.orchestrator.retry import backoff_seconds, should_retry
from financial_statements_engine.orchestrator.schema import ORCHESTRATOR_EVENTS, STAGES, WORKSTREAM_ID
...
def test_permanent_failure(fse_tmp):
    base = _ok_stages()
...
def test_retry_api(fse_tmp):
    base = _ok_stages()

    def bad(wf):
        raise StageError("NETWORK", "down", transient=False)

    # permanent fail first
    fns = dict(base)
    fns["PARSE"] = bad
